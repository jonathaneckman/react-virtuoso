...
const components: Partial<Components> = {
  ...
  Group: ({ children, ...props }) => {
    return <div {...props} style={{ backgroundColor: "#000000", color: "#ffffff" }}>{children}</div>
  },
}
...
export default function App() {
  ...
  const getEvents = (startDateString: string, direction: 'before' | 'after') => {
    ...
    let end = new Date(startDateString);
    if (direction == 'after') {
  ...
  const loadCurrentAndUpcomingEvents = (startDate: string,
    ...
    currentAndUpcomingEvents.forEach((event: string) => {
      ...
      let existingDateGroup = updatedDateGroups.find((dateGroup: string, index: number) => {
  ...
  const loadPastEvents = (startDate: string,
    updatedEvents: EventRow[],
    updatedDateGroups: string[],
    updatedDateGroupCounts: number[]) => {
    ...
    let lastGroup = dateGroups[dateGroups.length - 1];
    let pastEvents = getEvents(startDate, 'before');
    ...
    pastEvents.forEach((event: string) => {
      ...
      while (eventsGroup < moment(lastGroup).subtract(1, 'd').startOf('day').toISOString()) {
        let emptyGroup = moment(lastGroup).subtract(1, 'd').startOf('day').toISOString();
  ...
  useEffect(() => {
    let updatedDateGroups = [];
    let updatedDateGroupCounts = [];
    let updatedEvents: EventRow[] = [];

    let currentAndFutureEventData = loadCurrentAndUpcomingEvents(
    ...
    let pastEventData = loadPastEvents(
      THIS_MOMENT.clone().startOf('day').toISOString(),
  ...
  const appendEvents = React.useCallback(() => {
    return setTimeout(() => {
        ...
        let updatedDateGroups = [...dateGroups];
        let updatedDateGroupCounts = [...dateGroupCounts];
  ...
  return (
    <GroupedVirtuoso
      ...
      groupContent={(index) => {
        let groupMoment = moment(dateGroups[index]);
